...
本项目采用**首次登录自动初始化管理员**的简易上云逻辑：
1. **首次登入**：新部署项目未创建任何管理员时，在登录页面使用**任意账户**（无需确认是否为邮箱格式，例如 `admin`）和**任意密码**提交登录，系统会自动将该账号密码初始化为默认管理员账户并完成登录。
2. **密码变更**：登录进入控制台后，可以在 Web 界面的个人中心/设置中，按照正常逻辑（提供旧密码与新密码）随时自由更改账号与密码。
